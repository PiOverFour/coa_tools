...
func _enter_tree():
	...
	json_info = import_window.get_node("container/json_info/text")

	check_merge = import_window.get_node("container/check_merge_scenes")

	label_dst_scene = import_window.get_node("container/path_destination_file/label")
	label_src_scene = import_window.get_node("container/path_source_file/label")

	warning_dialog = import_window.get_node("container/warning_dialog")
	...
	btn_load_json = import_window.get_node("container/button_source")
	btn_load_json.connect("pressed",self,"_open_src_dialog")

	btn_dst_file = import_window.get_node("container/button_destination")
...
func _open_src_dialog():
	json_info.mouse_filter = json_info.MOUSE_FILTER_IGNORE
	src_dialog.popup_centered_ratio()
	src_scene = null
	#src_dialog.set_position(import_window.get_position())
...
func _src_dialog_confirm(path):
	json_info.clear()
	json_info.mouse_filter = json_info.MOUSE_FILTER_STOP
	json_path = path
	var json = File.new()
...
func _open_dst_dialog():
	...
	dst_scene = null
	#dst_dialog.set_position(import_window.get_position())
...
func replace_node(node, replace_with):
	if node.is_class("Node2D") or node.is_class("Sprite"):
		node.set_position(replace_with.get_position())
		node.set_rotation(replace_with.get_rotation())
...
func show_warning_dialog(title="",msg=""):
	...
	var pos = import_window.get_position() + import_window.get_size()*0.5 - warning_dialog.get_size()*0.5
		...
		if node["type"] == "SPRITE":
			...
			if copy_images:
				if json_path != "":
					...
					if !(dir.dir_exists(sprite_dir_path.plus_file("sprites"))):
						dir.make_dir(sprite_dir_path.plus_file("sprites"))
					if dir.file_exists(json_path.get_base_dir().plus_file(node["resource_path"])):
						var _src = json_path.get_base_dir().plus_file(node["resource_path"])
						var _dst = sprite_dir_path.plus_file(node["resource_path"])
						dir.copy(_src,_dst)

						### set sprite texture
						new_node.set_texture(load(sprite_dir_path.plus_file(node["resource_path"])))
